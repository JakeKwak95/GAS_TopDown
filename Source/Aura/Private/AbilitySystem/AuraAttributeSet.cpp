...
void UAuraAttributeSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
	...
	//	GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Yellow, FString::Printf(TEXT("Changed Health on %s, Health : %f"), *Props.TargetAvatarActor->GetName(),GetHealth()));
...
void UAuraAttributeSet::ShowFloatingText(const FEffectProperties& Props, const float LocalIncomingDamage, bool bBlockedHit, bool bCriticalHit) const
{
	...
	if (Props.SourceCharacter != Props.TargetCharacter)
	{
		...
		AuraPlayerController->ShowDamageNumber(LocalIncomingDamage, Props.TargetCharacter, bBlockedHit, bCriticalHit);
	}
}
